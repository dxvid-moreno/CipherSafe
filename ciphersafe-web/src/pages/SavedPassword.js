...
export default function SavedPasswords() {
  ...
  const fetchPasswords = async () => {
    try {
      ...
      setError(
        err.response?.data?.error || "No se pudieron cargar las contraseñas"
      );
    }
  };
  ...
  const exportPasswords = async (format) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_URL}/export/${userId}/${format}`,
        {
  ...
  const deletePassword = async (id) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/delete-password/${id}`
      );
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_API_URL}/update-password/${editingPassword.id}`,
        {
          tag: newTag,
  ...
  return (
    <div className="container mt-5">
      ...
      <button
        ...
        onClick={() => exportPasswords("csv")}
      >
        Exportar CSV
      </button>
      <button
        className="btn btn-outline-danger"
        onClick={() => exportPasswords("pdf")}
      >
        Exportar PDF
      </button>
        ...
        {passwords.length === 0 && !error ? (
          ...
          passwords.map((pw) => {
            ...
            return (
              ...
              >
                <div>
                  <strong>{pw.tag || "(sin etiqueta)"}</strong>
                  <br />
                  <small>{pw.created_at}</small>
                  <br />
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  Editar Contraseña: {editingPassword.tag || "(sin etiqueta)"}
                </h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                ...
                {editMessage && (
                  <div className="alert alert-info mt-3">{editMessage}</div>
                )}
              </div>
